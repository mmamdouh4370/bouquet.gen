...
const FlowerSuggestions = ({ id, onSave, onAdd, onRemove }) => {
      ...
      {!isSubmitted ? (
        ...
        <div className="flex flex-1 flex-wrap justify-center items-center rounded border-[0.1px] border-lightcream">
          {flowers.map((flower, index) => (
            <div
              key={index}
              className="relative flex-grow basis-0 h-[32rem] text-center flex flex-col justify-center px-4 max-w-m"
            >
              ...
              <div className="absolute inset-0 bg-black opacity-60 hover:opacity-90 hyphens-auto"></div>
              <div className="relative z-10 space-y-4 break-words flex flex-col">
                <h2 className="text-5xl text-cream font-bold break-words">
                  {flower.name}
                </h2>
                <p className="text-offwhite text-3xl z-10 break-words">
                  {flower.description}
                </p>
              </div>
            </div>
...
export default function FlowerSuggestionsArr() {
  ...
  const saveComponent = async (userId, flowerList) => {
    console.log(2);
    try {
